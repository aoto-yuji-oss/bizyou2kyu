...
// =========================================================
const GAS_API_URL = 'https://script.google.com/macros/s/AKfycbxNnydmPuXpyZp9guYqKpse6PpXjDFWhZJE7vkhf4OoXBjr2Boi4ZEt8B4_4idSVRrh/exec'; 
...
function showResults() {
    ...
    // 🔸 名前入力を促す（例：prompt）
    const playerName = prompt("名前または番号を入力してください（記録用）:");
    if (playerName) {
        sendScoreToGAS(playerName, score);
    }
}
...
async function sendScoreToGAS(name, score) {
    try {
        ...
        if (result.status === 'success') {
            console.log("スコア送信成功:", name, score);
        } else {
            console.error("スコア送信エラー:", result.message);
        }
    } catch (error) {
